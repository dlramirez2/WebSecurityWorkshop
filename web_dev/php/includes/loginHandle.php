  ...
  if(isset($_POST['submit'])){
   ...
   //check for valid username
    ...
    //}
        ...
        $sql = "SELECT * FROM medBlue.medBlueP WHERE userID=?;"; 
	...
	if(!mysqli_stmt_prepare($stmt,$sql)){
		...
		if($row = mysqli_fetch_assoc($result)){
			...
			if($hashPass != $row['pwdUser']){
				...
				//check that this has correct variable for row
				...
				$_SESSION['patientDOB'] = $row['DOB'];
				$_SESSION['patientMRN'] = $row['MRN'];
                		header("Location: ../patientHome.php?login=success"); 
				//change to patientHome
			}else{
